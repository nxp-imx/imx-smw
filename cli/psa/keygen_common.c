...
psa_key_usage_t parse_psa_usage_flags(const char *usage_str)
{
	...
	LOG_VERBOSE("Parsing PSA usage flags: %s", usage_str);
	...
	while (token) {
		...
		if (!strcasecmp(token, "sign")) {
			...
			LOG_ERROR("Unknown PSA usage flag: %s", token);
			free(usage_copy);
			return 0;
		}
	...
	LOG_VERBOSE("PSA usage flags parsed: 0x%08x", (unsigned int)flags);
 ...
 * @param buf         Output buffer for usage string
