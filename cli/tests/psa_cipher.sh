...
# Copyright 2026 NXP
#
# Cipher test suite - PSA backend (nxp_psa)
#
# Usage:
#   ./psa_cipher.sh
#
...
INPUT_FILE=$(mktemp /tmp/psa_cipher_input_XXXXXX.bin)

. "$SCRIPT_DIR/lib_cipher.sh"

# ---------------------------------------------------------------------------
# PSA-specific: persistent key tests
# PSA manages IV internally — no --iv option needed.
# ---------------------------------------------------------------------------
test_persistent_keys_psa() {
    ...
    # iv is always empty for PSA (managed internally)
    cases="
AES-128 CBC|AES|128|CBC|AES-CBC||16|$((base_id+1))
AES-128 CTR|AES|128|CTR|AES-CTR||16|$((base_id+2))
AES-128 ECB|AES|128|ECB|AES-ECB||16|$((base_id+3))
AES-128 CFB|AES|128|CFB|AES-CFB||16|$((base_id+4))
AES-128 OFB|AES|128|OFB|AES-OFB||16|$((base_id+5))
AES-256 CBC|AES|256|CBC|AES-CBC||16|$((base_id+6))
AES-256 CTR|AES|256|CTR|AES-CTR||16|$((base_id+7))
AES-256 ECB|AES|256|ECB|AES-ECB||16|$((base_id+8))
DES3-192 CBC|DES3|192|CBC|DES3-CBC||8|$((base_id+9))
DES3-192 ECB|DES3|192|ECB|DES3-ECB||8|$((base_id+10))
SM4-128 CBC|SM4|128|CBC|SM4-CBC||16|$((base_id+11))
SM4-128 CTR|SM4|128|CTR|SM4-CTR||16|$((base_id+12))
...
# ---------------------------------------------------------------------------
echo "╔══════════════════════════════════════════════════════════════════════════╗"
echo "║       NXP PSA CLI — Cipher Algorithm Test Suite                          ║"
echo "╚══════════════════════════════════════════════════════════════════════════╝"
