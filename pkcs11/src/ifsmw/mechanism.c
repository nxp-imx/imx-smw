...
#define TLS12_GCM_IV_SIZE	  12
#define TLS12_GCM_FIXED_IV_SIZE	  4
#define TLS12_GCM_PARTIAL_IV_SIZE 8

enum tls12_server_client {
	NO_TLS12_TYPE = 0,
	TLS12_SERVER = 1,
	TLS12_CLIENT = 2
};
...
static CK_RV op_maead(CK_SLOT_ID slotid, struct mentry *entry, void *args)
{
	 ...
	 */
	if (get_key_tls(obj) && params->state != OP_ONE_SHOT) {
		if (params->state == OP_UPDATE) {
			output_length = ctx->payload_length;
		} else if (params->state == OP_FINAL) {
			if (params->op_flag & CKF_ENCRYPT) {
				/* Ouput tag on encrypt final */
				output_length = ctx->tag_length;

				/* IV partially or fully generated */
				if (get_key_tls(obj) == TLS12_KEY &&
				    ctx->fixed_iv_length ==
					    TLS12_GCM_FIXED_IV_SIZE) {
					ctx->iv_length = TLS12_GCM_IV_SIZE;
					if (ADD_OVERFLOW(output_length,
							 TLS12_GCM_IV_SIZE,
							 &output_length)) {
						ret = CKR_ARGUMENTS_BAD;
						goto end;
	...
	/* IV partially or fully generated */
	if (get_key_tls(obj) == TLS12_KEY && params->poutput &&
	    (params->op_flag & CKF_ENCRYPT) &&
	    ctx->fixed_iv_length == TLS12_GCM_FIXED_IV_SIZE) {
		ctx->output = params->poutput;
		ctx->output_length = params->output_length;
		params->poutput = calloc(1, params->output_length);
		if (!params->poutput) {
			ret = CKR_HOST_MEMORY;
			goto end;
		}
	...
	if (params->op_flag & CKF_ENCRYPT) {
		if (get_key_tls(obj) == TLS12_KEY && params->poutput &&
			...
			if (ret == CKR_OK) {
				/*
				 * TLS 1.2 record is expecting IV
				 * to be at the beginning of the buffer.
				 * OpenSSL is waiting for the partial IV
				 * (8 last bytes of the IV).
				 */
				memcpy(ctx->output,
				       ctx->iv + TLS12_GCM_FIXED_IV_SIZE,
				       TLS12_GCM_PARTIAL_IV_SIZE);
				ctx->output += TLS12_GCM_PARTIAL_IV_SIZE;
				memcpy(ctx->output, params->poutput,
				       ctx->input_length);
			}
		...
		if (ctx->tag) {
			/* Add the tag */
			memcpy(ctx->output + ctx->input_length, ctx->tag,
			       ctx->tag_length);
		}
	}
