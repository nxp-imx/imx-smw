...
static int encrypt_init_bad_params(CK_FUNCTION_LIST_3_0_PTR pfunc)
{
	...
	ret = pfunc->C_MessageEncryptInit(0, &encrypt_mech, 0);
	if (CHECK_CK_RV(CKR_SESSION_HANDLE_INVALID, "C_MessageEncryptInit"))
		goto end;

	TEST_OUT("Check key handle NULL\n");
	ret = pfunc->C_MessageEncryptInit(sess, &encrypt_mech, 0);
	if (CHECK_CK_RV(CKR_KEY_HANDLE_INVALID, "C_MessageEncryptInit"))
		goto end;
	...
	if (CHECK_CK_RV(CKR_MECHANISM_INVALID, "C_MessageEncryptInit"))
		goto end;
...
static int decrypt_init_bad_params(CK_FUNCTION_LIST_3_0_PTR pfunc)
{
	int status = TEST_FAIL;
	...
	TEST_OUT("Check session NULL\n");
	ret = pfunc->C_MessageDecryptInit(0, &decrypt_mech, 0);
	if (CHECK_CK_RV(CKR_SESSION_HANDLE_INVALID, "C_MessageDecryptInit"))
		goto end;

	TEST_OUT("Check key handle are NULL\n");
	ret = pfunc->C_MessageDecryptInit(sess, &decrypt_mech, 0);
	if (CHECK_CK_RV(CKR_KEY_HANDLE_INVALID, "C_MessageDecryptInit"))
		goto end;
	...
	ret = pfunc->C_MessageDecryptInit(sess, &decrypt_mech,
					  secret_key_handle);
	if (CHECK_CK_RV(CKR_MECHANISM_INVALID, "C_MessageDecryptInit"))
		goto end;
	...
	ret = pfunc->C_MessageDecryptInit(sess, &decrypt_mech,
					  secret_key_handle);
	if (CHECK_CK_RV(CKR_MECHANISM_PARAM_INVALID, "C_MessageDecryptInit"))
		goto end;
...
static int encrypt_bad_params(CK_FUNCTION_LIST_3_0_PTR pfunc)
{
	int status = TEST_FAIL;
	...
	TEST_OUT("Check session NULL\n");
	ret = pfunc->C_EncryptMessage(0, NULL_PTR, 0, NULL_PTR, 0, data,
				      data_len, cipher, &cipher_len);
	if (CHECK_CK_RV(CKR_SESSION_HANDLE_INVALID, "C_EncryptMessage"))
		goto end;

	TEST_OUT("Check encrypted data length pointer NULL\n");
	ret = pfunc->C_EncryptMessage(sess, NULL_PTR, 0, NULL_PTR, 0, data,
				      data_len, cipher, NULL_PTR);
	if (CHECK_CK_RV(CKR_ARGUMENTS_BAD, "C_EncryptMessage"))
		goto end;
	...
	ret = pfunc->C_EncryptMessage(sess, NULL_PTR, 0, NULL_PTR, 0, NULL_PTR,
				      data_len, cipher, &cipher_len);
	if (CHECK_CK_RV(CKR_DATA_INVALID, "C_EncryptMessage"))
		goto end;
	...
	ret = pfunc->C_EncryptMessage(sess, NULL_PTR, 0, NULL_PTR, 0, data, 0,
				      cipher, &cipher_len);
	if (CHECK_CK_RV(CKR_DATA_LEN_RANGE, "C_EncryptMessage"))
		goto end;
	...
	ret = pfunc->C_EncryptMessage(sess, NULL_PTR, sizeof(iv), NULL_PTR, 0,
				      data, data_len, cipher, &cipher_len);
	if (CHECK_CK_RV(CKR_MECHANISM_PARAM_INVALID, "C_EncryptMessage"))
		goto end;
	...
	ret = pfunc->C_EncryptMessage(sess, iv, 0, NULL_PTR, 0, data, data_len,
				      cipher, &cipher_len);
	if (CHECK_CK_RV(CKR_MECHANISM_PARAM_INVALID, "C_EncryptMessage"))
		goto end;
...
static int decrypt_bad_params(CK_FUNCTION_LIST_3_0_PTR pfunc)
{
	int status = TEST_FAIL;
	...
	ret = pfunc->C_MessageDecryptInit(sess, &decrypt_mech, aes_hsecretkey);
	...
	ret = pfunc->C_DecryptMessage(sess, NULL_PTR, 0, NULL_PTR, 0, cipher,
				      data_len, NULL_PTR, &plaintext_len);
	...
	TEST_OUT("Check session NULL\n");
	ret = pfunc->C_DecryptMessage(0, NULL_PTR, 0, NULL_PTR, 0, cipher,
				      data_len, plaintext, &plaintext_len);
	if (CHECK_CK_RV(CKR_SESSION_HANDLE_INVALID, "C_DecryptMessage"))
		goto end;

	TEST_OUT("Check plaintext length 0\n");
	ret = pfunc->C_DecryptMessage(sess, NULL_PTR, 0, NULL_PTR, 0, cipher,
				      data_len, plaintext, NULL_PTR);
	if (CHECK_CK_RV(CKR_ARGUMENTS_BAD, "C_DecryptMessage"))
		goto end;
	...
	ret = pfunc->C_DecryptMessage(sess, NULL_PTR, 0, NULL_PTR, 0, NULL_PTR,
				      data_len, plaintext, &plaintext_len);
	if (CHECK_CK_RV(CKR_ENCRYPTED_DATA_INVALID, "C_DecryptMessage"))
		goto end;
	...
	ret = pfunc->C_DecryptMessage(sess, NULL_PTR, 0, NULL_PTR, 0, cipher, 0,
				      plaintext, &plaintext_len);
	if (CHECK_CK_RV(CKR_ENCRYPTED_DATA_LEN_RANGE, "C_DecryptMessage"))
		goto end;
	...
	ret = pfunc->C_DecryptMessage(sess, NULL_PTR, sizeof(iv), NULL_PTR, 0,
				      cipher, data_len, plaintext,
				      &plaintext_len);
	if (CHECK_CK_RV(CKR_MECHANISM_PARAM_INVALID, "C_DecryptMessage"))
		goto end;
	...
	ret = pfunc->C_DecryptMessage(sess, iv, 0, NULL_PTR, 0, cipher,
				      data_len, plaintext, &plaintext_len);
	if (CHECK_CK_RV(CKR_MECHANISM_PARAM_INVALID, "C_DecryptMessage"))
		goto end;
...
static int encrypt_decrypt_no_init(CK_FUNCTION_LIST_3_0_PTR pfunc)
{
	int status = TEST_FAIL;
	...
	TEST_OUT("Encrypt Message without init\n");
	ret = pfunc->C_EncryptMessage(sess, NULL_PTR, 0, NULL_PTR, 0, data,
				      data_len, cipher, &cipher_len);
	if (CHECK_CK_RV(CKR_OPERATION_NOT_INITIALIZED, "C_EncryptMessage"))
		goto end;

	TEST_OUT("Decrypt Message without init\n");
	ret = pfunc->C_DecryptMessage(sess, NULL_PTR, 0, NULL_PTR, 0, cipher,
				      cipher_len, data, &data_len);
...
static int encrypt_decrypt_multiple_init(CK_FUNCTION_LIST_3_0_PTR pfunc)
{
	int status = TEST_FAIL;
	...
	TEST_OUT("Check multiple message encrypt init with same mechanism\n");
	ret = pfunc->C_MessageEncryptInit(sess, &encrypt_decrypt_mech,
					  aes_hsecretkey);
	if (CHECK_CK_RV(CKR_OPERATION_ACTIVE, "C_MessageEncryptInit"))
		goto end;

	TEST_OUT("Check multiple msg encrypt init with different mechanism\n");
	...
	TEST_OUT("Check multiple message decrypt init with same mechanism\n");
	ret = pfunc->C_MessageDecryptInit(sess, &encrypt_decrypt_mech,
					  aes_hsecretkey);
	if (CHECK_CK_RV(CKR_OPERATION_ACTIVE, "C_MessageDecryptInit"))
		goto end;

	TEST_OUT("Check multiple msg decrypt init with different mechanism\n");
	...
	TEST_OUT("Check multiple message encrypt init with NULL mechanism\n");
	ret = pfunc->C_MessageEncryptInit(sess, NULL_PTR, aes_hsecretkey);
	if (CHECK_CK_RV(CKR_OK, "C_MessageEncryptInit"))
		goto end;

	TEST_OUT("Check multiple message decrypt init with NULL mechanism\n");
	ret = pfunc->C_MessageDecryptInit(sess, NULL_PTR, aes_hsecretkey);
	if (CHECK_CK_RV(CKR_OK, "C_MessageDecryptInit"))
		goto end;
...
static int encrypt_decrypt_generate_iv(CK_FUNCTION_LIST_3_0_PTR pfunc)
{
	...
	for (; i < ARRAY_SIZE(ulIvFixedBits); i++) {
		...
		ret = pfunc->C_EncryptMessage(sess, enc_dec_mech.pParameter,
...
static int encrypt_decrypt_aes(CK_FUNCTION_LIST_3_0_PTR pfunc)
{
	int status = TEST_FAIL;
	...
	CK_MECHANISM_TYPE aes_mech_type[] = {
		CKM_AES_CBC,	       CKM_AES_CFB128, CKM_AES_ECB, CKM_AES_CTR,
		CKM_AES_CTS,	       CKM_AES_OFB,    CKM_AES_GCM, CKM_AES_CCM,
		CKM_CHACHA20_POLY1305, CKM_AES_XTS
	};
	...
	for (; i < ARRAY_SIZE(aes_mech_type); i++) {
		...
		ret = pfunc->C_EncryptMessage(sess, enc_dec_mech.pParameter,
					      ...
					      encrypted_data,
					      &encrypted_data_len);
		if (CHECK_CK_RV(CKR_BUFFER_TOO_SMALL, "C_EncryptMessage"))
			goto end;
...
static int encrypt_decrypt_des(CK_FUNCTION_LIST_3_0_PTR pfunc)
{
	int status = TEST_FAIL;
	CK_RV ret = CKR_OK;
	CK_SESSION_HANDLE sess = 0;
	...
	for (; i < ARRAY_SIZE(des_mech_type); i++) {
		...
		/* Encrypt message when encrypted data buffer too small */
		ret = pfunc->C_EncryptMessage(sess, NULL_PTR, 0, NULL_PTR, 0,
					      data, data_len, encrypted_data,
					      &encrypted_data_len);
		if (CHECK_CK_RV(CKR_BUFFER_TOO_SMALL, "C_EncryptMessage"))
			goto end;
...
static int encrypt_decrypt_des3(CK_FUNCTION_LIST_3_0_PTR pfunc)
{
	int status = TEST_FAIL;
	CK_RV ret = CKR_OK;
	CK_SESSION_HANDLE sess = 0;
...
static int encrypt_decrypt_key_usage(CK_FUNCTION_LIST_3_0_PTR pfunc)
{
	int status = TEST_FAIL;
	CK_RV ret = CKR_OK;
	CK_SESSION_HANDLE sess = 0;
	...
	TEST_OUT("Initialize encrypt msg operation with decrypt secret key\n");
	ret = pfunc->C_MessageEncryptInit(sess, &encrypt_decrypt_mech,
					  hsecretkey_decrypt);
	if (CHECK_CK_RV(CKR_KEY_FUNCTION_NOT_PERMITTED, "C_MessageEncryptInit"))
		goto end;

	TEST_OUT("Initialize encrypt msg operation with encrypt secret key\n");
	ret = pfunc->C_MessageEncryptInit(sess, &encrypt_decrypt_mech,
					  hsecretkey_encrypt);
	if (CHECK_CK_RV(CKR_OK, "C_MessageEncryptInit"))
		goto end;
	...
	ret = pfunc->C_EncryptMessage(sess, NULL_PTR, 0, NULL_PTR, 0, data,
				      data_len, NULL_PTR, &encrypted_data_len);
	if (CHECK_CK_RV(CKR_OK, "C_EncryptMessage"))
		goto end;
	...
	TEST_OUT("Initialize decrypt message operation with non decrypt key\n");
	ret = pfunc->C_MessageDecryptInit(sess, &encrypt_decrypt_mech,
					  hsecretkey_encrypt);
	if (CHECK_CK_RV(CKR_KEY_FUNCTION_NOT_PERMITTED, "C_MessageDecryptInit"))
		goto end;

	TEST_OUT("Initialize decrypt msg operation with correct decrypt key\n");
	ret = pfunc->C_MessageDecryptInit(sess, &encrypt_decrypt_mech,
					  hsecretkey_decrypt);
	if (CHECK_CK_RV(CKR_OK, "C_MessageDecryptInit"))
		goto end;
