...
static const struct test_err_case {
	...
	ENUM_TO_STRING(SRC_CPY_CTX_NULL), ENUM_TO_STRING(DST_CPY_CTX_NULL),
	ENUM_TO_STRING(KDF_ARGS_NULL),
};
...
const struct error list_err[MAX_TEST_ERROR] = {
	...
	SET_ERR_CODE_AND_NAME(OBJECT_NOTFOUND, "OBJECT NOT FOUND"),
};
...
int util_check_result(struct subtest_data *subtest, int exp_status)
{
	...
	if (subtest->api_status != exp_status) {
		DBG_PRINT("Expected result is %s, got %s",
			  get_string_status(exp_status, subtest->api),
			  get_string_status(subtest->api_status, subtest->api));
		ret = 1;
	}
