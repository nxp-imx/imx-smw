				 ...
				 const uint8_t *cmd)
{
	...
	case TPM2_CC_PCR_Allocate:
		rc = handle_pcrallocate(tcti_smwtpm, header.tag, cmd,
					header.size);
		break;
	case TPM2_CC_CertifyCreation:
		rc = handle_certifycreation(tcti_smwtpm, header.tag, cmd,
					    header.size);
		break;
	case TPM2_CC_Unseal:
		rc = handle_unseal(tcti_smwtpm, header.tag, cmd, header.size);
		break;
	case TPM2_CC_MakeCredential:
		rc = handle_makecredential(tcti_smwtpm, header.tag, cmd,
					   header.size);
		break;
	case TPM2_CC_ActivateCredential:
		rc = handle_activatecredential(tcti_smwtpm, header.tag, cmd,
					       header.size);
		break;
	default:
		/* Unsupported command */
