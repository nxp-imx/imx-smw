...
/**
 * load_unmarshal() - Parse TPM2_Load command parameters from buffer.
 * @cmd:            Pointer to the command buffer containing the TPM2_Load command.
 * @cmd_size:       Size of the command buffer in bytes.
 * @input:          Pointer to the structure to be populated with unmarshaled
 *                  command parameters.
 * @nonce_caller:   Pointer to the structure to be populated with the caller's
 ...
 *                  handle extracted from the authorization area.
 *
 * This function unmarshals the TPM2_Load command parameters from the command
 * buffer after the TPM header. It extracts the parent handle, the private area
 * and the public area template. It also processes the authorization session
 * area to extract the session handle and caller nonce for HMAC validation.
 *
 * Return:
 * TSS2_RC_SUCCESS on successful parsing, or the corresponding error code on failure.
 */
